...
def prepare(prefix, source):
    ...
    with install_state.locked(prefix) as installed:
        ...
        from koinon import claude_guidance, claude_statusline, participant_instructions, revisions
        documents.put('prepared-checks', dict(version=1, plan=prepared['sha256'],
            ...
            claude_guidance=claude_guidance.plan(installed.config, prefix=prefix),
            participant_guidance=participant_instructions.plan(installed.config, prefix),
            runtime_revisions=dict(runtime_revision=revisions.runtime_revision(manifest=pair['source']))))
