...
def uninstall(prefix, state):
    ...
    if (state.config.get(claude_statusline.KEY) or {}).get('state') in ('enabled', 'pending'):
        print('Claude status line:', json.dumps(claude_statusline.remove(state, prefix)), flush=True)
