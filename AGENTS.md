...
memory service, and a fresh install with `--repo` also selects one; both then run
`memory_service.py ensure`, which starts it. Where no selection is made, start it
explicitly in a persistent managed session.
